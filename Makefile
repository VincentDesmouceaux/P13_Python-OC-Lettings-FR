...
# - Ignore commentaires (#) / lignes vides / lignes décoratives
define dotenv
...
PORT        ?= $(call dotenv,PORT)
DOCKER_REPO ?= $(call dotenv,DOCKER_REPO)
IMAGE_TAG   ?= $(call dotenv,IMAGE_TAG)

PORT        ?= 8000                   # fallback local

# Nom image/conteneur locaux
IMAGE      ?= oc-lettings
...
# Garde-fou pour les cibles “remote” (uniquement DOCKER_REPO requis)
...
.PHONY: run
run: sanitize-env ## Lance le conteneur local en détaché (port $(PORT))
	@ENV_OPTS=""; \
	if [ -f "$(SANITIZED_ENV)" ]; then ENV_OPTS="--env-file $(SANITIZED_ENV)"; fi; \
	docker run -d --name $(CONTAINER) -p $(PORT):8000 $$ENV_OPTS $(IMAGE)
...
.PHONY: run-remote
run-remote: sanitize-env ## Lance depuis l'image distante
	$(call require_remote)
	@ENV_OPTS=""; \
	if [ -f "$(SANITIZED_ENV)" ]; then ENV_OPTS="--env-file $(SANITIZED_ENV)"; fi; \
	docker run -d --name $(CONTAINER) -p $(PORT):8000 $$ENV_OPTS $(REMOTE_IMAGE)
...
run-remote-latest: sanitize-env ## Force un pull à chaque run (Docker récent)
	...
	@ENV_OPTS=""; \
	if [ -f "$(SANITIZED_ENV)" ]; then ENV_OPTS="--env-file $(SANITIZED_ENV)"; fi; \
	docker run --pull=always -d --name $(CONTAINER) -p $(PORT):8000 $$ENV_OPTS $(REMOTE_IMAGE)
